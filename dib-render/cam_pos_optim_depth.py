...
def main():
    ...
    phong_render = Renderer(HEIGHT, WIDTH, mode='Phong')
    vc_render = Renderer(HEIGHT, WIDTH, mode='VertexColor')
    ...
    rot_mat = kornia.quaternion_to_rotation_matrix(quat).to(device)
    trans = torch.tensor([0.5], dtype=torch.float)
    trans_xyz = torch.tensor([0, 0, 0.5], dtype=torch.float).to(device)
    camera_r_param, camera_t_param = make_camera_mat_from_quat(quat, trans)
    ...
    bs = len(vertices)
    material = np.array([[0.6, 0.6, 0.6], [1.0, 1.0, 1.0], [0.4, 0.4, 0.4]],
    ...
    xyzs = transform_pts_Rt_th(vertices[0], rot_mat[0], trans_xyz)[None]
    vc_predictions_ref, _, _ = vc_render(points=[vertices,
                                                 faces.long()],
                                         camera_params=camera_params,
                                         colors_bxpx3=xyzs)
    # vc_predictions_ref, _, _ = renderer(points=[vertices,
    #                                             faces.long()],
    #                                     camera_params=camera_params,
    #                                     colors_bxpx3=xyzs)
    import pdb
    ...
    vc_predictions_np = vc_predictions_ref[:, :, :, 2].cpu().numpy()[0]
    ...
    class Model(nn.Module):
        def __init__(self, renderer, image_ref, sil_ref, device, verticesc,
            ...
            quat = kornia.angle_axis_to_quaternion(
                torch.tensor([0.8 - np.pi, 0.0, 0.8 - np.pi],
                             dtype=torch.float))
