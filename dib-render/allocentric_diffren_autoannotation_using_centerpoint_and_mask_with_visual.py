...
from chamferdist import ChamferDistance as chamfer
import kornia
...
class Model(nn.Module):
    def __init__(self, renderer, depth_renderer, image_ref, sil_ref,
        ...
        self.azi = nn.Parameter(
            torch.from_numpy(np.array([-np.pi / 4.0],
                                      dtype=np.float32)).to(device))
        self.ele = nn.Parameter(
        ...
        self.camera_trans = nn.Parameter(
            torch.from_numpy(np.array([0.5], dtype=np.float32)).to(device))
        self.camera_uvc = nn.Parameter(
            torch.from_numpy(np.array([300.0, 300.0, 1.0],
                                      dtype=np.float32)).to(device))
        self.rot_xflip = torch.eye(3)
        self.rot_xflip[1, 1] = torch.tensor(-1)
        self.rot_xflip[2, 2] = torch.tensor(-1)
        ...
        vert_min = torch.min(verticesc)
        vert_max = torch.max(verticesc)
        self.colors = (verticesc - vert_min) / (vert_max - vert_min)
        ...
        # Chamfer Distance
        self.chamfer_dist = chamfer()

    def forward(self):
        #######
        # Allocentric
        #######
        p = torch.tensor([0.0, 0.0, 1.0], dtype=torch.float).to(self.device)
        q = torch.inverse(self.K) @ self.camera_uvc

        Rov = (rot_y(self.azi) @ rot_x(-self.ele) @ rot_z(
            self.tilt)) @ self.rot_xflip
        Rov = Rov.to(self.device)

        Rcv = rot_2vector(p, q, self.device)
        Tco = Rcv[:, 2] * self.camera_trans
        # Rov = ((rot_y(self.azi, self.device) @ rot_x(-self.ele, self.device))
        ...
        predictions, silhouette, _ = self.depth_renderer(
            points=[self.vertices, self.faces.long()],
            camera_params=camera_params,
        ...
        # Chamfer Distance
        # loss += 0.1 * self.chamfer_dist(
        #     self.points_ref[None], pre_points[None])
        loss += 0.0001 * self.chamfer_dist(self.points_ref[None],
                                           pre_points[None])

        # return loss, predictions, silhouette, None
        return loss, predictions, silhouette, pre_points
...
def main():
    ...
    # init_mat_co = np.array(
    #     [[-0.67963279, -0.04933497, 0.73189161, -0.06272623],
    #      [0.68088977, 0.32879057, 0.65443554, 0.09169828],
    #      [-0.27292562, 0.94311337, -0.18986518, 0.58473249],
    #      [0.0, 0.0, 0.0, 1.0]],
    #     dtype=np.float32)

    init_mat_co = np.array(
        [[-0.67963279, -0.04933497, 0.73189161, -0.09272623],
         [0.68088977, 0.32879057, 0.65443554, 0.05169828],
         [-0.27292562, 0.94311337, -0.18986518, 0.79473249],
         [0.0, 0.0, 0.0, 1.0]],
        dtype=np.float32)
    # -> -0.0912,  0.0521,  0.6282
    init_mat_co[:3, :3] = rot_z_np(np.pi / 4) @ rot_x_np(np.pi / 4 * 3)
    # init_mat_co[:3, :3] = rot_x_np(np.pi / 4 * 3)
    ...
    image_ref = model.image_ref.detach().cpu().numpy().squeeze()

    # Visualization
    import open3d as o3d
    _, _, _, pre_point = model()
    pcd1 = o3d.geometry.PointCloud()
    pcd1.points = o3d.utility.Vector3dVector(pre_point.detach().cpu().numpy())
    pcd2 = o3d.geometry.PointCloud()
    pcd2.points = o3d.utility.Vector3dVector(
        model.points_ref.detach().cpu().numpy())
    o3d.visualization.draw_geometries([pcd1, pcd2])
    points0 = pre_point.detach().cpu().numpy()
    np.save('init_points.npy', points0)
    ...
    for i in loop:
        with torch.autograd.set_detect_anomaly(True):
            ...
            loss, pre_img, pre_sil, pre_point = model()
            loss.backward()
            optimizer.step()
            ...
            print(loss)
        if i % 10 == 0:
            # image_init[0][np.where(image_init[0] != 0)] = 255
            image = pre_img[0].detach().cpu().numpy()
            image_mask = image_ref.copy()
            # image_mask[np.where(image_ref != 0)] = 1
            image_mask[np.where(image_ref != 0)] = 255
            image_mask = cv2.cvtColor(image_mask.astype(np.uint8),
                                      cv2.COLOR_BGR2GRAY)
            # import pdb
            # pdb.set_trace()
            # contours, hierarchy = cv2.findContours(
            #     image_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            im2, contours, hierarchy = cv2.findContours(
                image_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

            out = np.zeros_like(image_mask)
            # cv2.drawContours(out, contours, -1, 255, 3)
            cv2.drawContours(image, contours, -1, 1, 3)
            image = img_as_ubyte(image)
            writer.append_data(image)
            # # import pdb
            # # pdb.set_trace()
            # image[np.where(image < 0.1) and np.where(image_ref != 0)] = 1
            # # image[np.where(image < 0.1) and np.where(image_init != 0)] = 0.5
            # image = img_as_ubyte(image)
            # writer.append_data(image)
    writer.close()

    pcd1 = o3d.geometry.PointCloud()
    points1 = pre_point.detach().cpu().numpy()
    np.save('pre_points.npy', points1)
    pcd1.points = o3d.utility.Vector3dVector(points1)
    pcd2 = o3d.geometry.PointCloud()
    points2 = model.points_ref.detach().cpu().numpy()
    np.save('ref_points.npy', points2)
