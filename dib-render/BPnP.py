...
class BPnP(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            pts2d_i_np = np.ascontiguousarray(pts2d[i].detach().cpu()).reshape(
                (n, 1, 2))
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(
            ...
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_np,
                                     ...
                                     rvec=rvec0,
                                     tvec=T0)
            angle_axis = torch.tensor(rvec, device=device,
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            pts2d_flat = pts2d[i].clone().contiguous().view(
                -1).detach().requires_grad_()
            P_6d_flat = P_6d[i].clone().contiguous().view(
                -1).detach().requires_grad_()
            pts3d_flat = pts3d.clone().contiguous().view(
                -1).detach().requires_grad_()
            K_flat = K.clone().contiguous().view(-1).detach().requires_grad_()
            ...
            for j in range(m):
                ...
                R = kn.angle_axis_to_rotation_matrix(
                    P_6d_flat[0:m - 3].reshape(1, 3))

                P = torch.cat(
                    (R[0, 0:3, 0:3].reshape(3, 3), P_6d_flat[m - 3:m].reshape(
                        3, 1)),
                    dim=-1)
                KP = torch.mm(K_flat.reshape(3, 3), P)
                pts2d_i = pts2d_flat.reshape(n, 2).transpose(0, 1)
                pts3d_i = torch.cat((pts3d_flat.reshape(
                    n, 3), torch.ones(n, 1, device=device)),
                                    dim=-1).t()
                proj_i = KP.mm(pts3d_i)
                Si = proj_i[2, :].reshape(1, n)

                r = pts2d_i * Si - proj_i[0:2, :]
                coefs = get_coefs(P_6d_flat.reshape(1, 6),
...
class BPnP_m3d(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            pts2d_i_np = np.ascontiguousarray(pts2d[i].detach().cpu()).reshape(
                (n, 1, 2))
            pts3d_i_np = np.ascontiguousarray(pts3d[i].detach().cpu()).reshape(
                (n, 3))
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(
                ...
                rvec0 = np.array(ini_pose[i, 0:3].cpu().reshape(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().reshape(3, 1))
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_i_np,
                                     imagePoints=pts2d_i_np,
                                     cameraMatrix=K_np,
                                     distCoeffs=None,
                                     flags=cv.SOLVEPNP_ITERATIVE,
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            P_6d_flat = P_6d[i].clone().contiguous().view(
                -1).detach().requires_grad_()
            pts3d_flat = pts3d[i].clone().contiguous().view(
                -1).detach().requires_grad_()
            K_flat = K.clone().contiguous().view(-1).detach().requires_grad_()
...
class BPnP_fast(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            coefs = get_coefs(P_6d[i].reshape(1, 6), pts3d, K).detach()
...
def get_coefs(P_6d, pts3d, K):
    ...
    for k in range(2):
        torch.set_grad_enabled(True)
        y_grad = torch.autograd.grad(proj[:, :, k],
                                     y,
                                     vec,
                                     retain_graph=True,
                                     create_graph=True)
...
def batch_project(P, pts3d, K, angle_axis=True):
    ...
    if angle_axis:
        ...
        PM = torch.cat((R_out[:, 0:3, 0:3], P[:, 3:6].reshape(bs, 3, 1)),
                       dim=-1)
    else:
        PM = P
